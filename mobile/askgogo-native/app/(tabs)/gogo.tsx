...
function PriorityCard({item}:{item:AgentHomeItem}){const t=useTheme();const r=useRouter();const accent=item.kind==='watching'?'#5B8DD6':item.kind==='done'?'#3DB283':'#F26B1D';return <Pressable onPress={()=>r.push('/(tabs)/activity')} style={[s.card,{borderColor:t.hairline,backgroundColor:t.paper}]}><View style={s.charBox}><Gogo state={stateForItem(item.kind)} size={48}/></View><View style={{flex:1}}><Text style={[s.meta,{color:accent}]}>{labelFor[item.kind]}</Text><Text style={[s.cardTitle,{color:t.ink}]}>{item.title}</Text><Text style={[s.cardBody,{color:t.ink2}]} numberOfLines={3}>{item.body}</Text>{typeof item.progress==='number'?<View style={[s.track,{backgroundColor:t.surface}]}><View style={[s.fill,{width:`${Math.max(2,Math.min(100,item.progress))}%`} as any]}/></View>:null}<Text style={s.cardAction}>{item.actionLabel||'Open'} ›</Text></View></Pressable>}
...
const s=StyleSheet.create({wrap:{paddingHorizontal:20},header:{flexDirection:'row',alignItems:'center',justifyContent:'space-between'},brand:{fontSize:16,fontWeight:'800',letterSpacing:-.2},same:{fontSize:10},greeting:{fontSize:13,marginTop:34},gogo:{marginTop:14,width:76,height:76,justifyContent:'center'},hero:{fontSize:36,lineHeight:39,fontWeight:'700',letterSpacing:-1.15,marginTop:8,maxWidth:325},summary:{flexDirection:'row',flexWrap:'wrap',gap:12,marginTop:13,marginBottom:22},summaryItem:{flexDirection:'row',alignItems:'center',gap:5},dot:{width:5,height:5,borderRadius:99},summaryText:{fontSize:10,color:'#8F8983'},inline:{flexDirection:'row',alignItems:'center',gap:7,marginTop:8},inlineText:{fontSize:11},error:{fontSize:11,lineHeight:16,color:'#B34B3E',marginTop:8},sectionRow:{flexDirection:'row',justifyContent:'space-between',alignItems:'center',marginTop:26,marginBottom:9},sectionTitle:{fontSize:15,fontWeight:'700'},link:{fontSize:12,fontWeight:'700',color:'#F26B1D'},card:{flexDirection:'row',gap:12,borderWidth:1,borderRadius:18,padding:14},charBox:{width:48,height:48,alignItems:'center',justifyContent:'center'},meta:{fontSize:9,fontWeight:'800',letterSpacing:.75,marginBottom:5},cardTitle:{fontSize:17,lineHeight:21,fontWeight:'700'},cardBody:{fontSize:13,lineHeight:18,marginTop:5},cardAction:{fontSize:12,fontWeight:'700',color:'#F26B1D',marginTop:10},track:{height:4,borderRadius:99,overflow:'hidden',marginTop:10},fill:{height:'100%',borderRadius:99,backgroundColor:'#F26B1D'},empty:{borderWidth:1,borderRadius:18,padding:18},emptyTitle:{fontSize:17,fontWeight:'700'},emptyBody:{fontSize:13,marginTop:5}})
