...
export default function ConnectAskGogo() {
  ...
  async function sendCode() {
    ...
    if (digits.length !== 10) { setError('Enter your 10-digit WhatsApp number.'); return }
    const value = `${COUNTRY_CODE}${digits}`
  ...
  async function verify() {
    ...
    try {
      ...
      setState('linked')
      await Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success)
      setTimeout(()=>router.replace('/(tabs)/gogo'),350)
    } catch (e:any) {
      setState('code')
  ...
  return <SafeAreaView style={s.safe}><View style={s.wrap}>
    ...
    {(state==='phone'||state==='sending'||state==='error') && !link && <View style={s.card}>
      ...
      <View style={s.phoneRow}>
        <View style={s.country}><Text style={s.flag}>🇮🇳</Text><Text style={s.countryText}>{COUNTRY_CODE}</Text></View>
        <TextInput value={phone} onChangeText={v=>setPhone(v.replace(/\D/g,'').slice(0,10))} placeholder="88845 01501" keyboardType="number-pad" autoComplete="tel" maxLength={10} style={s.phoneInput}/>
      </View>
      <Text style={s.helper}>India is selected by default. Enter only your 10-digit mobile number.</Text>
