...
export async function POST(req: NextRequest) {
  ...
  if (action === 'add') {
    ...
    return NextResponse.json({ ok: true, reply: `✅ Added: _${text}_\n\nSay *tasks* to see all.` })
  }
  if (action === 'done') {
    ...
    return NextResponse.json({ ok: true, reply: `✅ Marked done: _${text}_` })
  }
  if (action === 'list') {
    ...
    const done = todos.filter(t => t.done).slice(0,2).map(t => `• ✓ ~${t.text}~`).join('\n')
    return NextResponse.json({ reply: `📋 *Your Tasks*\n\n${pending || '_All done!🎉_'}${done ? `\n\n_Recently done:_\n${done}` : ''}` })
  }
  if (action === 'clear') { await supabase.from('todos').delete().eq('whatsapp_id', phone).eq('done',true); return NextResponse.json({ ok: true, reply: '🗑 Cleared completed tasks.' }) }
