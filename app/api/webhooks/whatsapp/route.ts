...
` +
    ...
    if(jevIntent){
      const agentIntent=['watcher','reminder_read','reminder_mutation','email_read','email_mutation','travel_research','browser_action'].includes(jevIntent)
      if(agentIntent){
        const promotedAgent=await tryRunWhatsAppJevSpecialist({
          user:resolvedUser,
          text,
          messageId:inboundMessageSid||null,
          intent:jevIntent as 'watcher'|'reminder_read'|'reminder_mutation'|'email_read'|'email_mutation'|'travel_research'|'browser_action',
        })
        if(promotedAgent){
