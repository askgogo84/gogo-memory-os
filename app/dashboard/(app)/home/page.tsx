...
function Portal({ href, eyebrow, title, detail, accent }: { href: string; eyebrow: string; title: string; detail: string; accent: string }) {
  return (
    <Link href={href} className="group rounded-[24px] border border-white/55 bg-white/48 p-5 shadow-[0_18px_50px_rgba(62,35,18,0.045)] backdrop-blur-2xl transition duration-500 hover:-translate-y-1 hover:bg-white/68">
...
export default async function HomePage() {
  ...
  return (
    <div className="relative min-h-[calc(100vh-3.5rem)] overflow-hidden rounded-[34px] border border-gogo-ink/6 bg-[radial-gradient(circle_at_50%_18%,rgba(255,255,255,.95),rgba(255,247,236,.84)_38%,rgba(241,221,203,.62)_72%,rgba(230,209,229,.42)_100%)] shadow-[0_30px_90px_rgba(62,35,18,.07)]">
      ...
      <div className="pointer-events-none absolute inset-x-[18%] bottom-[-16%] h-[36%] rounded-[50%] bg-emerald-200/15 blur-[70px]" />
      ...
      <div className="relative mx-auto flex min-h-[calc(100vh-3.5rem)] max-w-[1180px] flex-col px-7 py-8 xl:px-12 xl:py-10">
        <div className="flex items-center justify-between gap-5">
          ...
          <div className="rounded-full border border-white/70 bg-white/55 px-3.5 py-2 text-[11px] font-semibold text-gogo-ink-2 backdrop-blur-xl">Everything is under control</div>
        </div>
        ...
        <section className="mx-auto flex w-full max-w-[820px] flex-1 flex-col items-center justify-center py-10 text-center">
          ...
          <div className="mt-7 flex flex-wrap items-center justify-center gap-2 text-[11px] font-semibold text-gogo-ink-3">
            <span className="rounded-full border border-white/65 bg-white/48 px-3 py-2 backdrop-blur">{pending} reminders</span>
            <span className="rounded-full border border-white/65 bg-white/48 px-3 py-2 backdrop-blur">{memoryCount} memories</span>
            <span className="rounded-full border border-white/65 bg-white/48 px-3 py-2 backdrop-blur">{listCount} lists</span>
          </div>
        </section>
        ...
        <section className="grid gap-3 md:grid-cols-3">
          ...
          <Portal href="/dashboard/memory" eyebrow="Memory" title="Find what you saved" detail="Documents, images and the things you asked Gogo not to forget." accent="text-emerald-700" />
