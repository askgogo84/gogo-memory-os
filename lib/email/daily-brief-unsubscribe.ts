...
function signature(telegramId: string, email: string) {
  ...
  return createHmac('sha256', key)
    .update(`daily-brief:${telegramId}:${email.trim().toLowerCase()}`)
    .digest('base64url')
}

export function buildDailyBriefUnsubscribeUrl(telegramId: string | number, email: string) {
  ...
  return `${base}/api/email/daily-brief/unsubscribe?${params.toString()}`
}

export function verifyDailyBriefUnsubscribeToken(telegramId: string, email: string, supplied: string) {
