...
function looksLikeSuccessfulValue(text: string, reply: string) {
  ...
  // Only nudge when bot actually delivered value — check the REPLY not just user text
  return (
    lowerReply.includes('✅') ||
    lowerReply.includes('🔔') ||
    lowerReply.includes('⏰') ||
    lowerReply.includes('reminder set') ||
    lowerReply.includes('reminders set') ||
    lowerReply.includes("i'll remind you") ||
    lowerReply.includes("i will remind you") ||
