...
function buildSvg(report: any) {
  ...
  const sensitivity = score(report, 'sensitivity_signs', 'low')
...
<svg xmlns="http://www.w3.org/2000/svg" width="1080" height="1350" viewBox="0 0 1080 1350">
  <defs>
    ...
    <style>
      ...
      .score{font:800 46px Arial,sans-serif;fill:#0f3329}
  ...
  <text x="830" y="92" class="pill">${esc(new Date(report.created_at).toLocaleDateString('en-IN', { day: '2-digit', month: 'short' }))}</text>
  ...
  <rect x="130" y="558" width="${Math.max(20, Math.min(320, Number(hydration) * 3.2))}" height="16" rx="8" fill="#2e8f75"/>
  ...
  <rect x="590" y="558" width="${Math.max(20, Math.min(320, Number(barrier) * 3.2))}" height="16" rx="8" fill="#c2994b"/>
...
export async function buildSkinReportCardReply(telegramId?: number) {
  ...
  const svg = buildSvg(latest)
  const mediaUrl = toDataUrl(svg)

  return {
    text:
      `✨ *Skin Report Card ready*\n\n` +
      `Use this as a premium visual summary of your latest Skin Check.\n\n` +
