...
async function relinkExistingPending(params:{actor:AgentActor;input:BookingCalendarInput;existing:any}){
  ...
  const [{error:runError},{error:approvalError}]=await Promise.all([
    supabaseAdmin.from('agent_runs').update({
      metadata_json:{...priorMeta,life_event_action_id:params.input.lifeEventActionId,proposedEvent},
      updated_at:now,
    }).eq('id',params.existing.runId).eq('telegram_id',tg),
    supabaseAdmin.from('agent_approvals').update({
      execution_payload:{...priorExecution,plan_type:PLAN_TYPE,action:'create_booking_calendar_event',lifeEventId:params.input.lifeEventId,lifeEventActionId:params.input.lifeEventActionId},
    }).eq('id',params.existing.approvalId).eq('telegram_id',tg),
  ])
...
export async function executeApprovedBookingCalendar(params:{actor:AgentActor;runId:string}){
  ...
  const writes:any[]=[
    ...
    supabaseAdmin.from('life_events').update({metadata_json:nextMeta,updated_at:now}).eq('id',linkedEvent.lifeEventId).eq('telegram_id',tg),
  ]
  if(lifeEventActionId)writes.push(supabaseAdmin.from('life_event_actions').update({status:'completed',updated_at:now,payload_json:{calendarEventId:data.id||eventId,calendarUrl:data.htmlLink||'',completedAt:now}}).eq('id',lifeEventActionId).eq('life_event_id',linkedEvent.lifeEventId).eq('telegram_id',tg))
