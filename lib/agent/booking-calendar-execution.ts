...
export type BookingCalendarInput={lifeEventId:string;lifeEventActionId?:string;title:string;startAt:string;endAt:string;timezone:string;location?:string|null;provider?:string|null;sourceUrl?:string|null;endEstimated?:boolean}
...
export async function executeApprovedBookingCalendar(params:{actor:AgentActor;runId:string}){
  ...
  const token=await calendarToken(params.actor),eventId=bookingCalendarEventId(ev.lifeEventId)
  ...
  const writes:any[]=[
    ...
    supabaseAdmin.from('life_events').update({metadata_json:nextMeta,updated_at:now}).eq('id',ev.lifeEventId).eq('telegram_id',tg),
  ]
  if(ev.lifeEventActionId)writes.push(supabaseAdmin.from('life_event_actions').update({status:'completed',updated_at:now,payload_json:{calendarEventId:data.id||eventId,calendarUrl:data.htmlLink||'',completedAt:now}}).eq('id',ev.lifeEventActionId).eq('life_event_id',ev.lifeEventId).eq('telegram_id',tg))
  await Promise.all(writes)
  await supabaseAdmin.from('agent_activity').insert({telegram_id:tg,run_id:params.runId,event_type:'booking_calendar_created',message:'Confirmed booking added to Google Calendar.',metadata_json:{life_event_id:ev.lifeEventId,life_event_action_id:ev.lifeEventActionId||null,event_id:data.id||eventId,reused}}).catch(()=>{})
