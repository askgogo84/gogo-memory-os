...
import { runLiveFlightBrowserTask } from './travel-browser-task'
...
function extractSegment(text: string, marker: 'from' | 'to' | 'in') {
  const stop = marker === 'from' ? '(?=\\s+(?:to|next|this|tomorrow|on|for|under|below|with|return|one-way|round-trip)\\b|$)' : marker === 'to' ? '(?=\\s+(?:from|next|this|tomorrow|on|for|under|below|with|return|one-way|round-trip)\\b|$)' : '(?=\\s+(?:next|this|tomorrow|on|for|under|below|with|from|to)\\b|$)'
...
function nextWeekRange(now: Date) {
  const base = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate())); const day = base.getUTCDay() || 7
  const monday = new Date(base); monday.setUTCDate(base.getUTCDate() + (8 - day)); const sunday = new Date(monday); sunday.setUTCDate(monday.getUTCDate() + 6)
...
function thisWeekRange(now: Date) {
  const base = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate())); const day = base.getUTCDay() || 7
  const sunday = new Date(base); sunday.setUTCDate(base.getUTCDate() + (7 - day))
...
function explicitDateFromText(text: string, now: Date) {
  ...
  if (m) { const d = new Date(Date.UTC(Number(m[1]), Number(m[2]) - 1, Number(m[3]))); if (!Number.isNaN(d.getTime())) return isoDay(d) }
  ...
  if (m) { const month = MONTHS[m[2].toLowerCase()]; const year = Number(m[3] || now.getUTCFullYear()); const d = new Date(Date.UTC(year, month, Number(m[1]))); if (d.getUTCFullYear() === year && d.getUTCMonth() === month && d.getUTCDate() === Number(m[1])) return isoDay(d) }
  ...
  if (m) { const month = MONTHS[m[1].toLowerCase()]; const year = Number(m[3] || now.getUTCFullYear()); const d = new Date(Date.UTC(year, month, Number(m[2]))); if (d.getUTCFullYear() === year && d.getUTCMonth() === month && d.getUTCDate() === Number(m[2])) return isoDay(d) }
  m = text.match(/\b(\d{1,2})[\/-](\d{1,2})[\/-](20\d{2})\b/)
  if (m) { const d = new Date(Date.UTC(Number(m[3]), Number(m[2]) - 1, Number(m[1]))); if (!Number.isNaN(d.getTime())) return isoDay(d) }
  return null
}
function normalizeWhen(text: string, now = new Date()) {
  ...
  if (/\bnext month\b/.test(t)) { const start = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth() + 1, 1)); const end = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth() + 2, 0)); return { label:new Intl.DateTimeFormat('en-GB',{month:'long',year:'numeric',timeZone:'UTC'}).format(start), search:`${fmtDate(start)} ${fmtDate(end)}`, startDate:isoDay(start), endDate:isoDay(end) } }
  if (/\btomorrow\b/.test(t)) { const d = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate() + 1)); return { label:fmtDate(d), search:fmtDate(d), startDate:isoDay(d), endDate:isoDay(d) } }
  const explicit = explicitDateFromText(text, now)
  if (explicit) { const d = new Date(`${explicit}T00:00:00Z`); return { label:fmtDate(d), search:fmtDate(d), startDate:explicit, endDate:explicit } }
...
export function buildTravelResearchContext(rawText: string, now = new Date()): TravelContext {
  const text = String(rawText || '').trim(); const t = text.toLowerCase(); const kind: 'flight' | 'hotel' = /\bhotel|hotels\b/.test(t) && !/\bflight|flights|airfare\b/.test(t) ? 'hotel' : 'flight'
  let origin = placeFrom(extractSegment(text, 'from')); let destination = placeFrom(extractSegment(text, 'to'))
  if (kind === 'flight' && (!origin || !destination)) { const pair = text.match(/\b([A-Za-z]{3}|[A-Za-z][A-Za-z .'-]{2,28})\s*(?:→|->| to )\s*([A-Za-z]{3}|[A-Za-z][A-Za-z .'-]{2,28}?)(?=\s+(?:next|this|tomorrow|on|for|under|below|with|one-way|round-trip)\b|$)/i); origin ||= placeFrom(pair?.[1]); destination ||= placeFrom(pair?.[2]) }
  if (kind === 'hotel' && !destination) destination = placeFrom(extractSegment(text, 'in'))
  const when = normalizeWhen(text, now); const routeLabel = origin && destination ? `${origin.code || origin.label} → ${destination.code || destination.label}` : destination ? destination.label : 'Travel search'
...
function formatLiveFlight(value:any,index:number){const price=Number.isFinite(Number(value.price))?`₹${Math.round(Number(value.price)).toLocaleString('en-IN')}`:'price unavailable';const stops=Number.isFinite(Number(value.stops))?(Number(value.stops)===0?'non-stop':`${Number(value.stops)} stop${Number(value.stops)===1?'':'s'}`):'';const timing=[value.departure,value.arrival].filter(Boolean).join(' → ');const bits=[price,value.airline,timing,stops].filter(Boolean);return `${index+1}. ${bits.join(' · ')}${formatRedemption(value)}${value.bookingLink?`\nBooking/provider link: ${value.bookingLink}`:''}`}
...
export async function tryRunTravelResearch(params:{actor:AgentActor;surface:AgentSurface;text:string}){
  ...
  const {data:step,error:stepError}=await supabaseAdmin.from('agent_steps').insert({telegram_id:String(tg),run_id:runId,ordinal:1,tool_name:'travel',title:'Obtain actual travel results',status:'running',input_json:{queries,context},output_json:{},started_at:now}).select('id').single()
