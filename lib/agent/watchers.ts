...
async function processProductStockWatcher(watcher:any, now:Date) {
  ...
  if (availability !== 'available') {
    // A previous availability alert is no longer valid once a fresh provider read
    // says unavailable/unknown. Retire any still-new idea linked to this watcher so
    // Autonomy Pulse cannot re-surface stale stock claims hours or days later.
