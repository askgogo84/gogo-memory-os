...
export async function closeBookingLink(params:{telegramId:number;text:string}):Promise<BookingClosureResult|null>{
  ...
  if(actionable){const watchAt=details.startAt?new Date(Math.max(Date.now()+60*60_000,new Date(details.startAt).getTime()-24*3600_000)).toISOString():new Date(Date.now()+6*3600_000).toISOString();await Promise.resolve(supabaseAdmin.from('life_event_actions').upsert({life_event_id:lifeEventId,telegram_id:String(params.telegramId),action_key:'booking-change-watch',action_type:'monitor',capability:'browser',title:'Watch booking source for time, venue or cancellation changes',due_at:watchAt,requires_approval:false,irreversible:false,payload_json:{bookingUrl:originalUrl,resolvedUrl,snapshot:details,notifyOnlyOnMaterialChange:true},updated_at:new Date().toISOString()},{onConflict:'life_event_id,action_key'})).catch(()=>{})}
