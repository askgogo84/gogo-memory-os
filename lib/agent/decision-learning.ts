...
export async function recordDecisionLearning(p:{actor:AgentActor;text:string;domain:DecisionDomain;handler:string;objectKind?:string|null;objectRef?:string|null;confidence?:number|null;outcome:DecisionOutcome;verified?:boolean;deliveryRef?:DeliveryLearningRef;correction?:string|null;decisionId?:string|null;firstRouteCorrect?:boolean|null}){const evidence=await hasDeliveryLearningEvidence(String(p.actor.legacyTelegramId),p.handler,p.objectKind,p.deliveryRef);const verified=p.verified===true&&evidence!==false;const metadata={schema:'same-brain-v2',decision_id:clean(p.decisionId,180)||null,first_route_correct:typeof p.firstRouteCorrect==='boolean'?p.firstRouteCorrect:null,domain:p.domain,user_text:isSecretShapedMemory(p.text)?'[sensitive turn withheld]':redactSecretShapedText(clean(p.text,800)),handler:clean(p.handler,100),object_kind:clean(p.objectKind,80)||null,object_ref:clean(p.objectRef,240)||null,confidence:finiteConfidence(p.confidence),outcome:p.outcome==='verified_success'&&!verified?'unknown':p.outcome,verified:p.outcome==='verified_success'&&verified,correction: p.correction ? (isSecretShapedMemory(p.correction)?'[sensitive correction withheld]':redactSecretShapedText(clean(p.correction,500))) : null};const {error}=await supabaseAdmin.from('agent_activity').insert({telegram_id:String(p.actor.legacyTelegramId),event_type:'decision_learning',message:`Same Brain v2: ${p.domain} / ${p.handler} / ${metadata.outcome}`,metadata_json:metadata});if(error)throw new Error('decision_learning_write_failed')}
