...
export function SideRail() {
  ...
  function NavGroup({ title, links }: { title: string; links: NavItem[] }) {
    return (
      <div className="mt-5">
        <div className="px-3 text-[8.5px] font-extrabold uppercase tracking-[0.20em] text-gogo-ink-4">{title}</div>
          ...
          {links.map((item) => {
            ...
            return (
              <Link
                ...
                className={`group relative flex items-center gap-3 rounded-[15px] border px-3 py-2.5 text-[11px] font-semibold transition duration-200 active:scale-[.985] ${active ? 'dashboard-nav-active text-gogo-ink' : 'border-transparent text-gogo-ink-2 hover:border-gogo-ink/7 hover:bg-gogo-surface/80 hover:text-gogo-ink'}`}
              >
                <span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-[11px] text-[11px] transition ${active ? 'bg-gogo-teal text-white shadow-[0_7px_16px_rgba(21,122,110,.18)]' : 'bg-gogo-surface/76 text-gogo-ink-3 ring-1 ring-gogo-ink/6 group-hover:text-gogo-teal'}`}>{item.icon}</span>
                <span className="min-w-0 flex-1 truncate">{item.label}</span>
                {active && <span className="h-2 w-2 rounded-full bg-gogo-teal shadow-[0_0_0_4px_rgba(21,122,110,.10)]" aria-hidden="true" />}
              </Link>
            )
          })}
        </div>
  ...
  return (
    <aside className="dashboard-rail hidden h-screen w-[220px] shrink-0 border-r border-gogo-ink/8 px-3.5 py-4 lg:flex lg:flex-col xl:w-[230px]">
      <Link href="/dashboard/home" className="dashboard-brand mx-0.5 flex items-center gap-3 rounded-[20px] border border-gogo-ink/7 bg-gogo-surface/72 px-3 py-3 backdrop-blur-xl">
