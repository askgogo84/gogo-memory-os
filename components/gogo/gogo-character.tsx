...
type Props = {
  ...
  animate?: boolean
  showStatus?: boolean
  /** Calm-state hands are opt-in. Breathing always rests both hands. */
  hands?: boolean
  className?: string
}
...
export function GogoCharacter({state='calm',size=72,dark=false,animate=true,showStatus,hands=false,className=''}:Props){
  ...
  const status=(showStatus ?? (active || state==='approval')) && state!=='done' && size>=40
  const bodyStroke=dark?'rgba(255,255,255,.12)':'rgba(70,45,30,.10)'
  const hairStroke=dark?'rgba(255,251,243,.32)':'none'
  const ringStroke=dark?'#201B18':'#F5F3EE'
  const statusSpeed=state==='watching'?'2.4s':'1.6s'
  ...
  return <span className={`inline-block shrink-0 leading-none ${className}`} style={{width:size,height:size}} aria-label={`Gogo — ${state}`} role="img">
    <svg viewBox="0 0 120 120" width="100%" height="100%" style={{overflow:'visible',display:'block'}}>
      ...
      {state==='secure'&&<path d="M60 6 C80 14 96 16 108 18 C108 60 96 92 60 114 C24 92 12 60 12 18 C24 16 40 14 60 6Z" fill="#F6F1E8" fillOpacity={dark?.06:.5} stroke={dark?'#F6F1E8':'#2C1A13'} strokeOpacity=".5" strokeWidth="2.5" strokeDasharray="6 5" style={{transformBox:'fill-box',transformOrigin:'center',animation:animate?'gogoGlow 4s ease-in-out infinite':'none'}}/>}
      ...
      <g style={{transformBox:'fill-box',transformOrigin:'center 90%',animation:animate?ANIM[state]:'none',opacity:state==='sleeping'?.82:1}}>
        <g fill={`url(#${hairId})`} stroke={hairStroke} strokeWidth="1.6"><circle cx="24" cy="52" r="9"/><circle cx="28" cy="40" r="10.5"/><circle cx="38" cy="30" r="11"/><circle cx="51" cy="23" r="11.5"/><circle cx="66" cy="22" r="12"/><circle cx="81" cy="28" r="11"/><circle cx="92" cy="39" r="10.5"/><circle cx="96" cy="52" r="9"/><circle cx="60" cy="32" r="13"/><circle cx="44" cy="40" r="10"/><circle cx="78" cy="38" r="10"/></g>
        ...
        {(state==='breathing'||(state==='calm'&&hands))&&<><ellipse cx="30" cy="98" rx="7" ry="5" fill="#F3E7D3" stroke="#E4D3B9"/><ellipse cx="90" cy="98" rx="7" ry="5" fill="#F3E7D3" stroke="#E4D3B9"/></>}
      ...
      {status&&<circle cx="98" cy="102" r="7" fill="#2FAE6A" stroke={ringStroke} strokeWidth="3" style={{animation:animate?`gogoPulse ${statusSpeed} ease-in-out infinite`:'none'}}/>}
    </svg>
  </span>
}
