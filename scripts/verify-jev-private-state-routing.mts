...
assert.match(bridge,/parseAutonomyCommand\(params\.text\)\)return null/)
assert.match(bridge,/capabilityIsOff\(actor\.legacyTelegramId,capability\)/)
assert.match(bridge,/conversationPersisted:true/)
assert.match(bridge,/dispatchThroughSameBrain/)
assert.match(wa,/!promotedAgent\.conversationPersisted/)
