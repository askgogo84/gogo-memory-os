...
assert.match(bridge,/tryGetProductStockWatchStatusFromCommand/)
assert.match(bridge,/tryCreateInboxTriageWatchFromCommand/)
assert.match(bridge,/tryCreateProductStockWatchFromCommand/)
...
assert.ok(bridge.indexOf('tryGetProductStockWatchStatusFromCommand({ actor') < bridge.indexOf('tryCreateProductStockWatchFromCommand({ actor'),'product stock status must beat new watch creation')
assert.ok(bridge.indexOf('tryCreateInboxTriageWatchFromCommand({ actor') < bridge.indexOf('tryRunBrowserCommand({ actor'),'inbox triage watch must beat generic browser/planner routing')
assert.ok(bridge.indexOf('tryCreateProductStockWatchFromCommand({ actor') < bridge.indexOf('tryRunBrowserCommand({ actor'),'product stock watch must precede generic browser routing')
