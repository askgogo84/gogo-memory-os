...
assert.match(whatsappBridge,/tryRunRestaurantReservation/)
assert.ok(whatsappBridge.indexOf('const restaurantReservation = await') < whatsappBridge.indexOf('const appointmentRecovery = await'), 'WhatsApp agent restaurant workflow must outrank appointment workflow')
